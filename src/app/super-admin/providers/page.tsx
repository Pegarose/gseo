  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {providers.map((p) => (
          <div key={p.provider} className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col justify-between">
            <div className="p-6 space-y-4">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <h3 className="font-bold text-lg text-gray-900">{p.provider}</h3>
                  <p className="text-xs text-gray-400 font-mono bg-gray-50 px-2 py-1 rounded inline-block">Provider: {p.provider.toLowerCase()}</p>
                </div>
            ...
            <div className="px-6 py-3 bg-gray-50 border-t border-gray-150 text-[10px] text-gray-400 font-mono">
              Son Kontrol: {new Date(p.lastCheckedAt).toLocaleString('tr-TR')}
            </div>
          </div>
        ))}
      </div>
