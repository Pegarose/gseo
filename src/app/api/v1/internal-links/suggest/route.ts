...
async function handler(req: NextRequest, context: AuthenticatedContext) {
  ...
  if (!sourceUrl || typeof sourceUrl !== 'string') {
    return errorResponse('Missing or invalid field: sourceUrl', 'VALIDATION_ERROR', 400, { field: 'sourceUrl' }, context.requestId);
  }
  ...
  // Find candidate target pages within the same site from recent snapshots
  ...
  const suggestions = candidates
    ...
    .map((c) => {
      ...
      const relevance = candidateText.split(/\W+/).filter((w: string) => w.length > 3 && sourceText.includes(w)).length;
      return {
  ...
  // Run linking module if HTML was provided to detect local issues
