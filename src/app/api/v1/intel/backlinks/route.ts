...
const FEATURE_KEY = 'vebapi.backlinkdata';
...
async function handler(req: NextRequest, context: AuthenticatedContext) {
  ...
  try {
    ...
    return errorResponse(
      err instanceof Error ? err.message : 'VebAPI backlink request failed',
      'PROVIDER_ERROR',
      502,
      { provider: 'vebapi' },
      context.requestId
