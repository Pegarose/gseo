...
interface Props {
  ...
  platformIntelEnabled: boolean;
}

export default function DashboardShell({ children, platformIntelEnabled }: Props) {
  const pathname = usePathname();
  const [sidebarOpen, setSidebarOpen] = useState(false);

  const navigation = platformIntelEnabled
    ? [baseNavigation[0], baseNavigation[1], intelligenceNav, ...baseNavigation.slice(2)]
    : baseNavigation;
          ...
          <nav className="space-y-1">
            {navigation.map((item) => {
              const isActive =
                ...
                (item.name === 'Intelligence' && pathname.startsWith('/dashboard/intelligence'));
              return (
                <Link
                  key={item.name}
                  href={item.href}
                ...
                >
                  <item.icon
                    className={`w-5 h-5 ${isActive ? 'text-indigo-700' : 'text-gray-400'}`}
                  />
                  {item.name}
                </Link>
              );
