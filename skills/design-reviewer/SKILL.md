...
description: 使用三维框架（系统设计、运行模拟、工程实现）系统性审查架构 and 系统设计文档，作为 challenge 工作流中的规范契约设计证据层。产出按严重度分级的发现，关联 to 具体文档段落。
---
...
1. **加载文档 (必须)**: 读取 `ARCHITECTURE.md`、所有 `SYSTEM_DESIGN/*.md` 以及所有 `ADR/*.md`。
...
|----|------|--------|----------|------|------|------|
| DR-01 | 系统设计 | Critical | ARCHITECTURE.md §X | 边界定义冲突，两个系统职责重叠 | 实现阶段职责漂移、返工风险高 | 重新划清系统边界并更新引用 |
| DR-02 | 运行模拟 | High | SYSTEM_DESIGN/... §Y | 故障传播路径未定义 | 级联失败时无法收敛 | 增加超时/降级/重试策略 |
