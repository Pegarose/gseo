...
8. **输出**：保存到 `.prismx/arch/v{N}/TASKS.md`
...
> 可引用的文档类型：
>
> - `ARCHITECTURE.md` §章节 - 系统边界、依赖关系
> - `PRD.md` - 需求定义、User Story
> - `ADR/ADR-XXX.md` - 架构决策、技术约束
> - `SYSTEM_DESIGN/{system}.md` §章节 - 接口契约、数据模型
> - 好: `SYSTEM_DESIGN/auth.md` §JWT 签发
> - 差: "JWT 相关设计"（无具体文档引用）
...
> **输出路径**: 任务清单应保存到 `.prismx/arch/v{N}/TASKS.md`，由调用方 (blueprint workflow) 指定具体的 `v{N}` 版本号。
