#!/usr/bin/env python3
"""
query_graph.py — AST on-demand query tool

Reads the ast_nodes.json produced by extract_ast.py and provides multiple query modes,
outputting concise text easily consumed by agents.

Purpose:
  - Assist the REASON/OBJECT/EMIT stages of the PROBE workflow in generating cognitive files
  - Support bug investigation, change impact assessment, and refactoring analysis during development
...
class GitStats:
    """Query helper for git_stats.json. Optional loading; does not affect core AST queries."""
...
class ASTGraph:
    ...
    def resolve_to_module_id(self, query: str) -> str | None:
        """Resolve a file path or module id uniformly to a module id."""
        # Try directly as a module id
        if query in self.nodes_by_id and self.nodes_by_id[query]['type'] == 'Module':
            return query
        # Try as a file path (compatible with \\ and /)
        normalized = query.replace('\\', '/')
        if normalized in self.path_to_module_id:
            return self.path_to_module_id[normalized]
        # Fuzzy match: strip leading repo-relative path prefix
    ...
    def resolve_to_path(self, module_id: str) -> str | None:
        """Resolve a module id to a file path."""
    ...
    def query_file(self, file_query: str) -> str:
        """--file: View the complete structure and import list for a file."""
    ...
    def query_who_imports(self, module_query: str) -> str:
        ...
        # Also try looking up directly in imports_reverse (handles external package names, etc.)
        if not mid:
            # Might be a partial match (e.g. 'flask' in imports target)
            matches = set()
    ...
    def query_impact(self, file_query: str) -> str:
        """--impact: Impact radius analysis (upstream and downstream dependency overview)."""
        mid = self.resolve_to_module_id(file_query)
        if not mid:
            return f"[NOT FOUND] No module matching '{file_query}'"
    ...
    def query_hub_analysis(self, top_n: int = 10) -> str:
        """--hub-analysis: Identify high fan-in / high fan-out core nodes."""
    ...
    def query_summary(self) -> str:
        """--summary: Structural summary aggregated by top-level directory."""
        # Aggregate by first or second level directory
        dir_stats: dict[str, dict] = defaultdict(
            lambda: {'modules': 0, 'classes': 0, 'functions': 0, 'lines': 0,
                     'class_names': [], 'import_dirs': set()}
        )

        # Determine aggregation granularity: use the first 2 levels of the path
...
""",
    ...
    # Check that at least one query mode is specified
    ...
    try:
        ...
        # Skip possible stderr lines mixed in (e.g. [WARNING]), locate the first '{' to start parsing
