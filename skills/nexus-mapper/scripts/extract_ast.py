...
Purpose: Extract module/class/function structure from code repositories using Tree-sitter, outputting JSON to stdout
...
# ── Built-in language configuration: loaded from languages.json in the same directory ──
...
def _load_builtin_languages() -> tuple[dict[str, str], dict[str, dict[str, str]], dict[str, str]]:
    ...
    # Normalize queries: ensure each language has both struct and imports keys
...
def _load_language_customizations(
    ...
    """
    Load and merge language customization configurations.
    
    Priority: CLI --language-config > CLI --add-* arguments > built-in configuration
    ...
    # Then load the --language-config file (if provided), which has the highest priority
...
def _load_languages(
    ...
    """
    Load Tree-sitter language objects, returning a {lang_name: Language} dictionary.
    Prefers tree-sitter-language-pack (160+ languages); falls back to single-language packages if unavailable.
    """
    try:
        ...
        # Fallback to Python single-language package only
        try:
    ...
    if not languages:
        sys.stderr.write("[ERROR] No language parsers available. Please install tree-sitter-language-pack\n")
        sys.exit(1)
    return languages


def _file_module_id(repo_path: Path, file_path: Path) -> str:
    """Convert a file path to a dot-separated module ID.
    Example: src/nexus/api/routes.py → src.nexus.api.routes
             src/core/parser.hpp   → src.core.parser
    """
    rel = file_path.relative_to(repo_path)
    parts = list(rel.parts)
    stem = Path(parts[-1]).stem  # Remove the extension
    parts[-1] = stem
    # Python special case: merge __init__ into the package path
...
def extract_file(
    ...
    """Parse a single source file, returning (nodes, edges, errors)"""
...
def collect_source_files(
    ...
    """Collect all source files of known languages in the repo, skipping excluded directories.
...
def apply_max_nodes(
    ...
    """
    When node count exceeds max_nodes, prioritize keeping Module/Class and truncate Function nodes.
